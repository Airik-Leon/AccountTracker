angular.module('appModule')
    .factory('accountService', function ($http){
        var service = {}; 
        var BASE_URL = 'http://localhost:8080/MVCProject/users'
        ...
        service.updateTransaction = function (tran) {
            return $http({
                method: 'PUT',
                url: BASE_URL + '/' + tran.user.id + '/accounts/' + tran.account.id+ '/transactions/' + transaction.id, 
                headers: {
                    'content-type': 'application/json'
                },
                data: tran 
        ...
        service.indexAccountTypes = function () {
            return $http({
                method: 'GET', 
                url: 'http://localhost:8080/MVCProject/accountType'
            })
        }
        service.indexTransactionTypes = function () {
            return $http({
                method: 'GET', 
                url: 'http://localhost:8080/MVCProject/transactionType'
            })
        }
        return service; 
    }); 